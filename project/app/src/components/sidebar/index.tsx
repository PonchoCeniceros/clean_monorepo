      ...
      <div className={`text-sm h-screen bg-baseDark text-white fixed flex flex-col justify-between transition-all duration-200 ${isMinimized ? 'w-20' : 'w-64'}`}>
        <div>
          ...
          <nav className="flex flex-col space-y-2 py-2">
            <h3 className={headerStyle}>{isMinimized ? 'S1' : 'Submódulo 1'}</h3>
            <Link className={sectionStyle} to="1">{isMinimized ? 'V1' : 'Vista 1'}</Link>
            <Link className={sectionStyle} to="2">{isMinimized ? 'V2' : 'vista 2'}</Link>
            <h3 className={headerStyle}>{isMinimized ? 'S2' : 'Submódulo 2'}</h3>
            <Link className={sectionStyle} to="1">{isMinimized ? 'V1' : 'Vista 1'}</Link>
            <Link className={sectionStyle} to="2">{isMinimized ? 'V2' : 'vista 2'}</Link>
          </nav>
        </div>
